...
| serviceProviderType | Um tipo a ser usado como provedor de serviços para injeção de dependências. Deve ser uma implementação de `IServiceProvider` e ter um construtor público com um único parâmetro do tipo `IServiceProvider`. | ServiceProvider |
